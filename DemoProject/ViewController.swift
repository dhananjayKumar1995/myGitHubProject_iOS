...
extension ViewController{
   func toggleDemo(){
        ...
        demoButton.setTitle("ToggleMe", for: .normal)
        demoButton.backgroundColor = .gray
        demoButton.frame = CGRect(x: view.center.x, y: view.center.y, width: 100, height: 100)
        demoButton.addTarget(self, action: #selector(toggling), for: .touchUpInside)
        demoButton.center = view.center
        view.addSubview(demoButton)
    }
    
    @objc func toggling(button:UIButton){
        let check = button.isSelected.toggleWithReturnValue()
        check ? (button.backgroundColor = .red) : (button.backgroundColor = .blue)
    }
}
